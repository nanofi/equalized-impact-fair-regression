...
def main(target='./results/params/train/jcn/'):
    os.makedirs(target, exist_ok=True)

    model_path = "./results/models/jcn/"
    os.makedirs(model_path, exist_ok=True)
    log_path = "./logs/train/jcn/"
    os.makedirs(log_path, exist_ok=True)

    defaults = {
        'name': 'jcn',
        'method': './method/jcn_method_eo.py',
        'learning_seed': 1,
        'depth': 3,
        'shared_depth': 3,
        'hidden': 64,
    ...
    for dataset in datasets:
        for dataset_seed in range(10):
            for eps in np.linspace(0.01, 0.99, num=10).tolist():
